...
pub fn list(facts: Facts, out: *[max_items]Speedup) []const Speedup {
    ...
    out[n] = .{
        .name = "Кадр по размеру окна",
        .what = "декодер отдаёт кадр такого размера, какой показан, а не какой в файле",
        .cost = "снимок кадра берётся отдельно, в полном размере, и потому медленнее",
        .made_by = "Media Foundation, продвинутое видеопреобразование",
        .on = facts.boost,
        .works = facts.scaled,
    };
    n += 1;
    ...
    out[n] = .{
        .name = "Декодер в стороне от окна",
        .what = "окно просит кадр и не ждёт его; просьбы склеиваются, работает последняя",
        .cost = "пока новый кадр не готов, виден прежний",
        .made_by = "свой код",
    ...
    out[n] = .{
        .name = "Открытие по оглавлению",
        .what = "длина и дорожки читаются из оглавления файла, а не из всего файла",
        .cost = "",
        .made_by = "свой код",
        .on = true,
    };
    ...
    out[n] = .{
        .name = "Волна в стороне",
        .what = "звуковая волна считается не в потоке окна: клип появляется сразу",
        .cost = "волна дорисовывается через секунду-другую",
        .made_by = "свой код",
        .on = true,
    };
    n += 1;
    ...
    out[n] = .{
        .name = "Окно не рисует под кнопками",
        .what = "перерисовка не задевает панель: кнопки не мигают при воспроизведении",
        .cost = "",
        .made_by = "Windows, признак WS_CLIPCHILDREN",
        .on = true,
    };
    n += 1;
